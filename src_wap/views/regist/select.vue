            ...
            return new Promise((resolve,reject) => {
                this.getToken().then((res) => {
                    ...
                    for(let i=0; i<this.licenseImgFile.length; i++){            // 主图
                        let random_name =res.data.basePath + 'shop/' + this.random_string(6) + '_' + new Date().getTime() + '.' + this.licenseImgFile[i].name.split('.').pop()
                    ...
                    for(let i=0; i<this.productImgFile.length; i++){            // 主图
                        let random_name =res.data.basePath + 'shop/' + this.random_string(6) + '_' + new Date().getTime() + '.' + this.productImgFile[i].name.split('.').pop()

...
export default {
  ...
  created(){
    ...
    this.searchGroup().then((res) =>{
      ...
      this.getProInfo().then((res) =>{
        ...
        this.copyShareTitle = '【仅剩'+(this.groupData.groupPurchase.groupNumber - this.groupData.groupPurchase.offerNumber)+'个名额】我超低价拼了'+this.detailData.productExtInfo.title+'，快来和我一起拼团吧'+window.location.href.split('&')[0]+'点击链接，参与拼团【来自茶帮通茶友分享】';
      })
      //获取为您推荐集合
  ...
  methods: {
   ...
   setWxShare(wxConfig){
      ...
      let shareLink = 'https://'+location.host+'/#/share?orderId='+this.orderId;
